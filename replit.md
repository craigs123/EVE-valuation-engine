...
- **STAC API Critical Fixes v2.8.0**: MAJOR RESTORATION - Fixed completely broken satellite data access that was returning "Unknown" for all coordinates. Root causes: (1) Wrong year selection - was using 2020 data (404 errors) instead of working 1992 data, (2) Incorrect typo correction - working URLs have `go_espg.4326` typo, "fixed" URLs don't exist, (3) Year filtering blocked pre-2010 data. Now correctly uses 1992 ESA land cover data with exact STAC asset URLs matching working implementation. ESA code 11 successfully extracted at agricultural test coordinates (40.1°N, -87.91°W).
