...
**Urban coefficients were replaced on 2026-08-09** with revised medians from the latest ESVD release; the block now sums to 16,982 Int$/ha/yr against 2,206,166 previously. The superseded `recreation` coefficient of 2,157,864 was erroneous — roughly 8,800x the $245/ha/yr target recorded in its own provenance file — and constituted ~99% of every urban valuation EVE produced. Urban totals fall roughly 100-fold as a result, so urban analyses saved before this date are not comparable with later ones. The Urban sections of `esvd_coefficient_study_mappings.txt` and `detailed_esvd_study_value_mappings.txt` are marked SUPERSEDED: per-study mappings for the revised values have not yet been supplied.
