...
# Title and header  
st.title("🌱 Ecosystem Valuation Engine v2.4.0")
...
with col2:
    ...
    if st.session_state.get('analysis_results'):
        ...
        with col_metrics1:
            ...
            st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Total Value:</strong> ${total_value:,.0f} /year</p>', unsafe_allow_html=True)
            ...
            if water_area and water_area > 0 and total_area > 0:
                st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Land Area Analyzed:</strong> {land_area:,.0f} hectares</p>', unsafe_allow_html=True)
                st.caption(f"🌊 Water area excluded: {water_area:,.0f} ha ({water_area/total_area*100:.1f}% of total)")
            else:
                st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Area Size:</strong> {land_area:,.0f} hectares</p>', unsafe_allow_html=True)
        
        with col_metrics2:
            value_per_ha = results.get('value_per_ha', 0)
            st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Value per Hectare:</strong> ${value_per_ha:,.0f} /ha/year</p>', unsafe_allow_html=True)
            ...
            if results.get('forest_classification'):
                ...
                if forest_info and forest_info.get('detected_type'):
                    ...
                    st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>🌲 Forest Type:</strong> {ecosystem_display}</p>', unsafe_allow_html=True)
                else:
                    st.markdown('<p style="font-size:10px; margin:2px 0;"><strong>Predominant Ecosystem Type:</strong> Classification Pending</p>', unsafe_allow_html=True)
            else:
                ecosystem_type = results.get('ecosystem_type', 'Unknown')
                ecosystem_display = ecosystem_type.replace('_', ' ').title()
                st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Predominant Ecosystem Type:</strong> {ecosystem_display}</p>', unsafe_allow_html=True)
        ...
        if area_ha and area_ha > 0:
            st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Area Size:</strong> {area_ha:.0f} hectares</p>', unsafe_allow_html=True)
        else:
            st.markdown('<p style="font-size:10px; margin:2px 0;"><strong>Area Size:</strong> Calculating...</p>', unsafe_allow_html=True)
...
if st.session_state.analysis_results:
    ...
    if analysis_mode == "Summary Analysis":
        ...
        with col_metrics[2]:
            ...
            if 'ecosystem_composition' in results.get('metadata', {}):
                ...
                st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Primary Ecosystem:</strong> {dominant_type}</p>', unsafe_allow_html=True)
                st.caption(f"Mixed area: {len(composition)} ecosystem types")
            else:
                st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Predominant Ecosystem Type:</strong> {results["ecosystem_type"]}</p>', unsafe_allow_html=True)
    ...
    if 'esvd_results' in results:
        ...
        if has_categories:
            ...
            if total_val > 0:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Total Ecosystem Services:</strong> ${total_val:,.0f}/year</p>', unsafe_allow_html=True)
                    st.caption(f"${per_ha:.0f} per hectare annually")
                with col2:
                    regional_factor = esvd_data.get('regional_adjustment_factor', 1.0)
                    st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Regional Adjustment:</strong> {regional_factor:.2f}x</p>', unsafe_allow_html=True)
    ...
    if 'esvd_results' in results and results.get('ecosystem_type') == 'multi_ecosystem':
        if 'ecosystem_results' in results.get('esvd_results', {}):
            ...
            with col_total1:
                st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Combined Total Value:</strong> ${results["total_value"]:,.0f}/year</p>', unsafe_allow_html=True)
            ...
            with col_total2:
                ...
                st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Combined Value per Hectare:</strong> ${combined_per_ha:,.0f}/ha/year</p>', unsafe_allow_html=True)
                st.caption("Weighted average across all ecosystems")
            
            with col_total3:
                num_ecosystems = len(ecosystem_results)
                st.markdown(f'<p style="font-size:10px; margin:2px 0;"><strong>Predominant Ecosystem Types Detected:</strong> {str(num_ecosystems)}</p>', unsafe_allow_html=True)
