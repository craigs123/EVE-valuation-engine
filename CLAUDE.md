...
- **EEI: zero ≠ missing.** A `0.0` integrity score is a real measurement (built-up land scores zero) and is applied. Anything that is *not* a real reading — fabricated (demo) data, a reported measurement failure, or a real response carrying no value at the pixel (open ocean, coverage gaps) — takes `NO_DATA_FALLBACK_INTACTNESS_PCT` (50%, the midpoint) and the affected ecosystems are named in the EEI panel and the PDF. **Changed 2026-09-18:** the third case previously fell through to an optimistic 100% on the grounds that 50% would be "as unfounded as 100%" for open ocean. True, but it proves too much — if both are unfounded, prefer the one that does not systematically flatter the total, and do not split two identical "we don't know" cases on which way the upstream service happened to fail. The upstream EEI service must never use truthiness tests on integrity scores (that bug reported every city centre as null until 2026-08)
...
- **Four coefficient tables are live** — `_ESVD_LOG_WINSORISED` / `_ESVD_MEDIAN` / `_ESVD_MEAN`, plus `_ESVD_HYBRID` ("Evidence-guarded", `log_winsorised_guarded`), derived at import from the first two by substituting the median wherever `0 < n < 15`. Selected under Analysis Settings → Valuation Basis (`st.session_state['esvd_statistic']`, still **`log_winsorised` by default** — the guarded basis is *recommended* but not the default, so nothing re-baselines; users are prompted once before their first analysis and the answer is stored on `users.valuation_basis`). Sample counts live in `_ESVD_SAMPLE_COUNTS`; `test_calculations.py` checks them against the `n=` in every coefficient comment. The guarded basis is **not uniformly conservative** — in 11 cells the median is the higher figure — so never describe it as such. Every `PrecomputedESVDCoefficients()` call site picks this up via `resolve_esvd_statistic()` — that's deliberate, so don't thread a `statistic=` argument through call sites just to make it explicit. The run's basis is stamped into results as `coefficient_statistic`, shown on the dashboard above the totals, and printed in the PDF summary table
